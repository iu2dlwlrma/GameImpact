...
namespace GameImpact.Vision
{
    ...
    {
        /// <summary>游戏 UI 综合预处理（推荐用于游戏界面） 通过提取多种颜色通道并合并，提升各种颜色文字的识别率</summary>
        public static Mat PreprocessGameUI(Mat image)
        {
            ...
            using var scaled = scale > 1.0 ? bgr.Resize(new Size(), scale, scale, InterpolationFlags.Cubic) : bgr.Clone();
        ...
        /// <summary>提取亮度高于阈值的像素（白色/浅色文字）</summary>
        ...
        /// <summary>提取指定 HSV 颜色范围的像素</summary>
        ...
        /// <summary>简单预处理（灰度 + CLAHE + 二值化）</summary>
