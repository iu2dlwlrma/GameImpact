...
/// 全局静态日志工具类。
/// <para>
/// 提供统一的日志输出入口，内部委托 <see cref="ILogger"/> 实现实际写入。
/// 支持两种日志通道：
/// <list type="bullet">
///   <item><description><b>常规日志</b>（Debug/Info/Warn/Error）—— 仅输出到日志后端（控制台/文件）并触发 <see cref="OnLogMessage"/> 事件。</description></item>
///   <item><description><b>屏幕日志</b>（DebugScreen/InfoScreen/WarnScreen/ErrorScreen）—— 在常规日志的基础上额外触发 <see cref="OnScreenLogMessage"/> 事件，
///     供 UI 层（如 MainWindow 日志面板、OverlayWindow 叠加日志）订阅并显示。</description></item>
/// </list>
/// </para>
...
public static class Log
{
    ...
    /// <summary>
    /// 初始化日志系统，注入 <see cref="ILoggerFactory"/>。
    ...
    /// <summary>
    /// 获取指定类型 <typeparamref name="T"/> 的强类型 Logger。
    ...
    public static void Debug(string message, params object[] args) => LogInternal(LogLevel.Debug, message, args);

    /// <summary>输出 Info 级别日志（纯文本）</summary>
    public static void Info(string message) => LogInternal(LogLevel.Information, message);

    /// <summary>输出 Info 级别日志（带结构化占位符）</summary>
    public static void Info(string message, params object[] args) => LogInternal(LogLevel.Information, message, args);

    /// <summary>输出 Warn 级别日志（纯文本）</summary>
    public static void Warn(string message) => LogInternal(LogLevel.Warning, message);

    /// <summary>输出 Warn 级别日志（带结构化占位符）</summary>
    public static void Warn(string message, params object[] args) => LogInternal(LogLevel.Warning, message, args);

    /// <summary>输出 Error 级别日志（纯文本）</summary>
    public static void Error(string message) => LogInternal(LogLevel.Error, message);

    /// <summary>输出 Error 级别日志（带结构化占位符）</summary>
    public static void Error(string message, params object[] args) => LogInternal(LogLevel.Error, message, args);
    ...
    public static void DebugScreen(string message) => LogScreenInternal(LogLevel.Debug, message);

    /// <summary>Debug 级别日志，同时输出到屏幕（带结构化占位符）</summary>
    public static void DebugScreen(string message, params object[] args) => LogScreenInternal(LogLevel.Debug, message, args);

    /// <summary>Info 级别日志，同时输出到屏幕（纯文本）</summary>
    public static void InfoScreen(string message) => LogScreenInternal(LogLevel.Information, message);

    /// <summary>Info 级别日志，同时输出到屏幕（带结构化占位符）</summary>
    public static void InfoScreen(string message, params object[] args) => LogScreenInternal(LogLevel.Information, message, args);

    /// <summary>Warn 级别日志，同时输出到屏幕（纯文本）</summary>
    public static void WarnScreen(string message) => LogScreenInternal(LogLevel.Warning, message);

    /// <summary>Warn 级别日志，同时输出到屏幕（带结构化占位符）</summary>
    public static void WarnScreen(string message, params object[] args) => LogScreenInternal(LogLevel.Warning, message, args);

    /// <summary>Error 级别日志，同时输出到屏幕（纯文本）</summary>
    public static void ErrorScreen(string message) => LogScreenInternal(LogLevel.Error, message);

    /// <summary>Error 级别日志（带结构化占位符），同时输出到屏幕</summary>
    public static void ErrorScreen(string message, params object[] args) => LogScreenInternal(LogLevel.Error, message, args);

    /// <summary>
    /// Error 级别日志（含异常信息），同时输出到屏幕
    /// </summary>
    /// <param name="ex">捕获的异常对象</param>
    /// <param name="message">附加的描述消息</param>
    ...
    /// <summary>
    /// 常规日志核心方法（带占位符）：写入日志后端 + 触发 OnLogMessage。
    ...
    /// <summary>
    /// 屏幕日志核心方法（纯文本）：写入日志后端 + 触发 OnLogMessage + 触发 OnScreenLogMessage。
    ...
    /// <para>
    /// 所有带 params 的 XxxScreen 均委托此方法。
    /// 格式化仅执行一次，避免旧实现中 Format 被重复调用的性能浪费。
    /// </para>
    /// </summary>
    /// <param name="level">日志级别</param>
    /// <param name="message">包含 {Xxx} 占位符的消息模板</param>
    ...
    private static string GetLevelAbbreviation(LogLevel level) => LevelAbbreviations.GetValueOrDefault(level, "???");

    /// <summary>
    /// 触发 <see cref="OnLogMessage"/> 事件，通知所有常规日志订阅者。
    ...
    /// <summary>
    /// 触发 <see cref="OnScreenLogMessage"/> 事件，通知所有屏幕日志订阅者。
    /// <para>内部使用 try-catch 防止订阅者异常影响日志调用方。</para>
    /// </summary>
    /// <param name="level">日志级别缩写（DBG / INF / WRN / ERR）</param>
    /// <param name="message">格式化后的日志消息</param>
    ...
    /// <para>
    /// 使用正则表达式匹配 {Xxx} 占位符，逐个替换为对应参数的字符串表示。
    /// 与手动 IndexOf 方式不同，即使参数的 ToString() 结果中包含 '{' 也不会导致后续替换错乱。
    /// </para>
    /// <example>
    /// Format("鼠标点击 ({X}, {Y})", 100, 200) → "鼠标点击 (100, 200)"
    /// </example>
    /// </summary>
...
/// 空 Logger 实现（非泛型版本）。
/// <para>
/// 当 <see cref="Log"/> 尚未通过 <see cref="Log.Initialize"/> 初始化时，
/// 作为兜底返回，避免调用方出现 NullReferenceException。
/// 所有日志操作均为空操作（no-op）。
/// </para>
/// </summary>
...
/// 空 Logger 实现（泛型版本）。
/// <para>
/// 当 <see cref="Log"/> 尚未初始化时，由 <see cref="Log.For{T}"/> 返回，
/// 提供与 <see cref="ILogger{T}"/> 兼容的空操作实现。
/// </para>
/// </summary>
