...
{
    /// <summary>统一输入模拟器。调用 <see cref="SetWindowHandle"/> 后，键盘和鼠标操作均通过 PostMessage 投递到目标窗口，无需前台焦点，也不会移动鼠标光标。</summary>
    ...
    {
        /// <summary>后台点击（PostMessage）：向目标窗口发送 WM_LBUTTONDOWN/UP，无需移动光标。 适用于处理 WM 消息的窗口；对 DirectInput / Raw Input 游戏无效。</summary>
        bool BackgroundClickAt(int x, int y);

        /// <summary>前台点击（SendInput）：激活目标窗口，将鼠标移动到客户端坐标处后执行真实点击。 对所有游戏均有效，但会切换窗口焦点并移动鼠标光标。</summary>
