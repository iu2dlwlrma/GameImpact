...
/// <summary>
/// 设置页签构建器：将 Settings 模型按分组自动拆分成树形 SettingsPage 结构。
/// 用于 SettingsWindow 动态生成导航栏。
/// </summary>
public static class SettingsPageBuilder
{
    /// <summary>
    /// 将一个 Settings 模型拆分成带子页签的 SettingsPage 节点。
    /// 每个 SettingsGroup 生成一个子页签，各自独立的 AutoSettingsView。
    /// </summary>
    ...
    public static SettingsPage Build<T>(
        ISettingsProvider<T> settingsProvider,
        string title,
        string icon = "",
        int order = 0,
    ...
    public static SettingsPage CreateCategory(
        string title,
        string icon = "",
        int order = 0,
