...
/// <summary>
/// 通用设置视图：通过反射 Settings 模型上的 Attribute 自动生成 UI 控件。
/// 反射元数据通过 SettingsMetadataCache 缓存，只解析一次。
/// </summary>
...
public class AutoSettingsView<T> : UserControl where T : class, new()
{
    ...
    private void BuildUi()
    {
        ...
        foreach (var group in groups)
        {
            ...
            if (!string.IsNullOrEmpty(group.GroupName))
            {
                ...
                string headerText = string.IsNullOrEmpty(group.Icon) ? group.GroupName : $"{group.Icon} {group.GroupName}";
    ...
    /// <summary>
    /// 根据属性类型和 Attribute 配置创建对应的 UI 控件
    /// </summary>
...
#region 控件创建
    ...
    private ComboBox CreateComboBoxControl(SettingsItemMetadata meta)
    {
        ...
        if (propType.IsEnum)
        {
            ...
            {
                string name = enumValue?.ToString() ?? string.Empty;
                string displayText = optionsMap.TryGetValue(name, out string? custom) ? custom : name;
...
#region 值同步
    ...
    private void LoadSettingsToUi()
    {
        ...
        foreach (var kvp in m_controlMap)
        {
            ...
            object? value = property.GetValue(m_settings);
    ...
    private static void SetControlValue(FrameworkElement control, PropertyInfo property, object? value)
    {
        switch (control)
        {
            ...
            {
                string tagValue = value?.ToString() ?? string.Empty;
            ...
            {
                double numValue = Convert.ToDouble(value ?? 0);
    ...
    /// <summary>
    /// 控件值变更时：从控件读取值写回 Settings 模型并保存
    /// </summary>
    private void OnControlValueChanged(PropertyInfo property)
    {
        ...
        object? newValue = GetControlValue(control, property);
        if (newValue == null)
        {
            return;
        }
    ...
    private static object? GetControlValue(FrameworkElement control, PropertyInfo property)
    {
        ...
        switch (control)
        {
            ...
            case TextBox textBox:
            {
                ...
                {
                    return int.TryParse(text, out int intVal) ? intVal : null;
                }

                if (propType == typeof(double))
                {
                    return double.TryParse(text, NumberStyles.Float, CultureInfo.InvariantCulture, out double doubleVal) ? doubleVal : null;
                }

                if (propType == typeof(float))
                {
                    return float.TryParse(text, NumberStyles.Float, CultureInfo.InvariantCulture, out float floatVal) ? floatVal : null;
                }
...
#region 工具方法
    ...
    /// <summary>
    /// 解析 Options 字符串为 字典（值 → 显示文本）。
    /// 格式："显示文本:值|显示文本:值" 或 "值|值"（此时显示文本 = 值）
    /// </summary>
    private static Dictionary<string, string> ParseOptions(string options)
    {
        ...
        string[] pairs = options.Split('|', StringSplitOptions.RemoveEmptyEntries);
