...
/// <summary>
/// Direct3D 11 辅助类，提供设备创建、纹理操作和格式转换功能
/// </summary>
public static class Direct3D11Helper
{
    ...
    public static bool IsHdrEnabledForWindow(nint hWnd)
    {
        try
        {
            var hMonitor = Vanara.PInvoke.User32.MonitorFromWindow(
                hWnd, Vanara.PInvoke.User32.MonitorFlags.MONITOR_DEFAULTTONEAREST);
            ...
            for (int adapterIdx = 0; ; adapterIdx++)
            {
                ...
                using (adapter)
                {
                    for (int outputIdx = 0; ; outputIdx++)
                    {
                        ...
                        using (output)
                        {
                            if (output.Description.MonitorHandle != hMonitor) continue;
    ...
    /// <summary>
    /// 从 IDirect3DSurface 创建 SharpDX Texture2D
    /// </summary>
    ...
    /// <summary>
    /// 将 GPU 纹理数据填充到 OpenCV Mat（BGRA32 格式）
    /// </summary>
    public static bool FillMat(this Texture2D staging, Device device, Texture2D source, Mat destMat, ResourceRegion? region = null)
    {
        ...
        try
        {
            ...
            try
            {
                int w = staging.Description.Width;
    ...
    private static IDirect3DDevice CreateDirect3DDeviceFromSharpDXDevice(Device d3dDevice)
    {
        ...
        CreateDirect3D11DeviceFromDXGIDevice(dxgiDevice.NativePointer, out nint pUnknown);
    ...
    private static unsafe void CopyBgra(DataBox dataBox, Mat dest, int w, int h)
    {
        ...
        byte* dst = (byte*)dest.Data;
        int srcPitch = dataBox.RowPitch;
        int dstStep = (int)dest.Step();
        int rowBytes = w * 4;

        if (srcPitch == dstStep && dstStep == rowBytes)
        {
            System.Buffer.MemoryCopy(src, dst, (long)h * rowBytes, (long)h * rowBytes);
        }
        else
        {
            for (int y = 0; y < h; y++)
                System.Buffer.MemoryCopy(src + y * srcPitch, dst + y * dstStep, rowBytes, rowBytes);
        }
    }

    /// <summary>
    /// HDR (R16G16B16A16_Float) → BGRA32，使用 LUT 加速
    /// </summary>
    private static unsafe void ConvertHdrToBgra(DataBox dataBox, Mat dest, int w, int h)
    {
        ...
        Parallel.For(0, h, y =>
        {
            Half* srcRow = (Half*)(src + y * srcPitch);
...
/// <summary>
/// HDR→SDR 查找表（Hable Tone Mapping + sRGB Gamma）
/// </summary>
internal sealed class HdrToSdrLut
{
    ...
    private HdrToSdrLut()
    {
        ...
        float Hable(float x) => ((x * (A * x + C * B) + D * E) / (x * (A * x + B) + D * F)) - E / F;
        float whiteScale = 1f / Hable(W);

        for (int i = 0; i <= LutSize; i++)
        {
            float hdr = i * MaxHdrValue / LutSize;
            float mapped = Hable(Math.Max(0f, hdr)) * whiteScale;
    ...
    /// <summary>
    /// 将 HDR 线性值转换为 SDR sRGB 值
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public byte Convert(float hdrValue)
    {
        if (hdrValue <= 0f) return 0;
        if (hdrValue >= MaxHdrValue) return _lut[LutSize];
        return _lut[(int)(hdrValue * (LutSize / MaxHdrValue))];
    }
}

/// <summary>
/// IDirect3DSurface 的 DXGI 接口访问
/// </summary>
