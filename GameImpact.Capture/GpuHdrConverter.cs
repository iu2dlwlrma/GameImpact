...
{
    /// <summary>GPU 端 HDR→SDR 转换器，使用 Compute Shader 包含多重保护机制防止 GPU 错误导致系统死机</summary>
    /// <summary>GPU 端 HDR→SDR 转换器，使用 Compute Shader</summary>
    public class GpuHdrConverter : IDisposable
    {
        ...
        /// <summary>执行 HDR→SDR 转换，带完整的错误保护</summary>
        /// <summary>执行 HDR→SDR 转换，带完整的错误保护</summary>
        ...
        public Texture2D? Convert(Texture2D hdrTexture)
        {
            ...
            try
            {
                ...
                // 复制输入纹理
                context.CopySubresourceRegion(hdrTexture, 0, null, m_inputCopyTexture, 0);
