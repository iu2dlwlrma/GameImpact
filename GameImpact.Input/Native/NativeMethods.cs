...
{
    /// <summary>封装 Win32 User32.dll 原生 API 的 P/Invoke 声明。 提供窗口消息发送、坐标转换、输入模拟、窗口管理等底层能力。</summary>
    internal static partial class NativeMethods
    {
        ...
        /// <summary>根据窗口类名和/或窗口标题查找顶层窗口。</summary>
        ...
        /// <summary>向指定窗口投递（Post）一条异步消息，不等待目标窗口处理即返回。 适用于不需要等待返回结果的输入模拟场景（如按键、鼠标事件）。</summary>
        ...
        /// <summary>向指定窗口发送（Send）一条同步消息，阻塞等待目标窗口处理完毕后返回。</summary>
        /// <param name="hWnd">目标窗口句柄。</param>
        ...
        /// <summary>将屏幕坐标转换为指定窗口的客户区坐标。</summary>
        /// <param name="hWnd">目标窗口句柄。</param>
        ...
        /// <summary>将指定窗口的客户区坐标转换为屏幕坐标。</summary>
        /// <param name="hWnd">目标窗口句柄。</param>
        ...
        /// <summary>将键盘/鼠标输入事件合成注入到输入流中，等同于真实的物理输入。 这些事件会被发送到当前拥有键盘/鼠标焦点的窗口。</summary>
        ...
        /// <summary>将虚拟键码映射为扫描码，或将扫描码映射为虚拟键码。</summary>
        ...
        /// <summary>全局键盘模拟（旧版 API），将按键事件发送给当前前台焦点窗口（等同真实按键）。 推荐优先使用 <see cref="SendInput"/>，此方法保留用于兼容性场景。</summary>
        ...
        /// <summary>获取鼠标光标在屏幕上的当前位置（屏幕坐标）。</summary>
        ...
        /// <summary>将鼠标光标移动到屏幕上的指定位置。</summary>
        ...
        /// <summary>将指定窗口设置为前台窗口（激活并获得焦点）。 注意：系统对此 API 有限制，只有满足特定条件的进程才能成功调用。</summary>
        ...
        /// <summary>判断指定窗口是否处于最小化（图标化）状态。</summary>
        ...
        /// <summary>控制窗口的显示状态（显示、隐藏、最大化、最小化、还原等）。</summary>
        /// <param name="hWnd">目标窗口句柄。</param>
        ...
        /// <summary>将客户区坐标 (x, y) 打包为窗口消息的 lParam 格式。 低 16 位存放 x 坐标，高 16 位存放 y 坐标。 用于构造 WM_MOUSEMOVE、WM_LBUTTONDOWN 等鼠标消息的 lParam 参数。</summary>
        ...
        /// <summary>表示一个二维坐标点（对应 Win32 POINT 结构体）。 用于屏幕坐标与客户区坐标的相互转换。</summary>
        ...
        /// <summary>描述一个输入事件（对应 Win32 INPUT 结构体）。 通过 <see cref="Type"/> 字段区分键盘输入、鼠标输入或硬件输入。</summary>
        [StructLayout(LayoutKind.Sequential)]
        ...
        /// <summary>输入联合体（对应 Win32 INPUT 中的匿名 union）， 使用 FieldOffset(0) 使三种输入类型共享同一块内存。</summary>
        ...
        /// <summary>鼠标输入事件数据（对应 Win32 MOUSEINPUT 结构体）。</summary>
        [StructLayout(LayoutKind.Sequential)]
        ...
        /// <summary>键盘输入事件数据（对应 Win32 KEYBDINPUT 结构体）。</summary>
        [StructLayout(LayoutKind.Sequential)]
        ...
        /// <summary>硬件输入事件数据（对应 Win32 HARDWAREINPUT 结构体），一般很少使用。</summary>
        [StructLayout(LayoutKind.Sequential)]
