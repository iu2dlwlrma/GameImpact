...
/// <summary>
/// 输入相关诊断，用于排查 SetCursorPos / SendInput 被拦截时可能的原因。
/// </summary>
public static class InputDiagnostics
{
    /// <summary>
    /// 将当前进程已加载的模块（DLL）列表写入日志。
    /// 用于排查鼠标/键盘被拦截：常见拦截者包括罗技/雷蛇驱动、游戏覆盖层、远程桌面、安全软件等。
    /// </summary>
    ...
    /// <summary>
    /// 测试 SetCursorPos 是否可用，并记录结果与 GetLastError。
    /// </summary>
