...
namespace GameImpact.Input
{
    public partial class SendInputSimulator : IKeyboardInput
    {
        ...
        /// <summary>按下并释放指定按键（单次按键）</summary>
        /// <param name="key">虚拟键码</param>
        /// <returns>当前实例，支持链式调用</returns>
        ...
        /// <summary>执行组合键操作（修饰键+主键）</summary>
        ...
        /// <summary>输入文本字符串（使用 Unicode 编码）</summary>
        ...
        /// <summary>通过 SendInput 发送虚拟键码</summary>
        /// <param name="key">虚拟键码</param>
        ...
        {
            var flags = isKeyUp ? NativeMethods.KeyEventFlags_KeyUp : 0;
        ...
        /// <summary>通过 SendInput 发送扫描码</summary>
        /// <param name="scan">扫描码</param>
        /// <param name="isKeyUp">是否为释放动作</param>
        /// <returns>成功发送的事件数量</returns>
        private static uint SendInputScanCode(ushort scan, bool isKeyUp)
        {
            var flags = NativeMethods.KeyEventFlags_Scancode;
            if (isKeyUp)
            {
        ...
        /// <summary>
        ///     通过 PostMessage 投递 WM_KEYDOWN / WM_KEYUP 到目标窗口。 lParam 格式 (32 bit): bits  0-15 : repeat count (1) bits 16-23 : scan code bit  24    : extended key flag (0) bits 25-28 : reserved (0) bit  29    : context code (0) bit  30    : previous key state (0=down, 1=up) bit  31    : transition state
        ///     (0=down, 1=up)
        /// </summary>
        /// <param name="key">虚拟键码</param>
        /// <param name="scan">扫描码</param>
